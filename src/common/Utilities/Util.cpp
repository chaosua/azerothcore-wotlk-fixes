...
std::wstring GetMainPartOfName(std::wstring const& wname, uint32_t declension)
{
    ...
    static std::array<std::array<std::wstring const*, 25>, 6> const dropEnds = { {
            //  ола         це         ця         га          ґа          ий         ка         ко         ок         іж         іл         ер        ра         ру        з         к         а         е         я         у         є         і        ь        й        о
            { &ola_End,   &tse_End,  &tsya_End, &ga_End,    &gha_End,  &ii_End,    &ka_End,   &ko_End,   &ok_End,   &izh_End,  &il_End,   &er_End,  &ra_End,   &ru_End,  &z_End,   &k_End,   &a_End,   &e_End,   &ya_End,  &u_End,   &ye_End,  &i_End,  &y_end,  &j_End,  &o_End  }, // 1-й відмінок (називний)
            //  оли         ожа        ола         ого        яти         за         ги         ґи         ця         ці         ка         ки        ої         ра        ру        ри        а         и         і         ї         я         р
            { &oly_End,   &ozha_End, &ola_End,   &ogo_End,  &yaty_end, &za_End,    &gy_End,    &ghy_End,  &tsya_End, &tsi_End,  &ka_End,  &ky_End,  &oi_End,   &ra_End,  &ru_End,  &ry_End,  &a_End,   &i_end,   &i_End,   &yi_End,  &ya_End,  &r_End,  nullptr, nullptr, nullptr }, // 2-й відмінок (родовий)
            //  ожу         олі        олу         ому        еві         єві        ові        яті        рам        ій         ці         цю        зу         зі        ку        ру        рі        е         ю         у         і         ї
            { &ozhu_End,  &oli_End,  &olu_End,   &omu_End,  &evi_End,  &ievi_End,  &ovi_End,  &yati_End, &ram_End,  &ij_End,   &tsi_End,  &tsy_End, &zu_End,   &zi_End,  &ku_End,  &ru_End,  &ri_End,  &e_End,   &yu_End,  &u_End,   &i_End,   &yi_End, nullptr, nullptr, nullptr }, // 3-й відмінок (давальний)
            //  ожа         ола        олу         ого        за          гу         ґу         це         цю         ка         ко         ку        ра         ру        іл        а         е         я         ю         у         о
            { &ozha_End,  &ola_End,  &olu_End,   &ogo_End,  &za_End,   &gu_End,    &ghu_End,  &tse_End,  &tsy_End,  &ka_End,   &ko_End,   &ku_End,  &ra_End,   &ru_End,  &il_End,  &a_End,   &e_End,   &ya_End,  &yu_End,  &u_End,   &o_End,   nullptr, nullptr, nullptr, nullptr }, // 4-й відмінок (знахідний)
            //  ожем        олом       олою        рами       гою         ґою        зом       цею        цем        кою        ком         ром       рою        ру        ом        ем        ею        єю        єм        ою        ям        им       ю
            { &ozhem_End, &olom_End, &oloyu_End, &ramy_End, &goyu_End, &ghoyu_End, &zom_End,  &tsey_End, &tsem_End, &kom_End,  &koyu_End, &rom_End, &royu_End, &ru_End,  &om_End,  &em_End,  &eyu_End, &iey_End, &yem_End, &oyu_End, &yam_End, &im_End, &yu_End, nullptr, nullptr }, // 5-й відмінок (орудний)
            //  ожі         ому        еві         ові        єві         яті        олі        рах        ру         рі         ку         ій        зі         ці        і         ї         ю         у 
            { &ozhi_End,  &omu_End,  &evi_End,   &ovi_End,  &ievi_End, &yati_End,  &oli_End,  &rah_End,  &ru_End,   &ri_End,   &ku_End,   &ij_End,  &zi_End,   &tsi_End, &i_End,   &yi_End,  &yu_End,  &u_End,   nullptr,  nullptr,  nullptr, nullptr, nullptr, nullptr, nullptr  }  // 6-й відмінок (місцевий)
        } };

    std::size_t const thisLen = wname.length();
    std::array<std::wstring const*, 25> const& endings = dropEnds[declension];
